...
SUBROUTINE negf_mixed(POT3D,charge_n,charge_p,ISDcurrent,IDScurrent,IDScurrentb,ss,gg,phonon,ext_iter)
...
COMPLEX(DP), ALLOCATABLE :: sigma_r_ph(:,:,:)
...
REAL(DP), ALLOCATABLE :: emin_yz(:),emax_yz(:),degeneracy(:),SCBA_error(:),SCBA_x(:),SCBA_f(:)
REAL(DP)              :: SCBA_scalar
REAL(DP)              :: n_bose_g
...
real(dp), allocatable :: pt(:), CR(:,:), Ry(:), Rz(:), dens_yz(:,:),dens_z(:,:), omega(:,:), derror(:,:), derror_old(:,:)
...
!!$do ix=1,1*Nrx
!!$write(501,*)(ix-1)*Dx*1d7
!!$end do
!!$do iy=1,NY+1
!!$   write(502,*)(iy-1)*Dy*1d7
!!$end do
!!$do iz=1,NZ+1
!!$   write(503,*)(iz-1)*Dz*1d7
!!$end do
!!$close(501)
!!$close(502)
!!$close(503)
...
!!$      call surf_interp(iy,iz,CR,tr)
!!$      write(134,*)(iy-1)*dy+to2_lft*deltay,(iz-1)*dz+to2_bot*deltaz,tr
!!$   end do
!!$   write(134,*)
!!$end do
!!$deallocate(CR)
!!$do iy=1,NTOT_Y
!!$do iz=1,NTOT_Z
!!$   write(135,*)(iy-1)*deltay,(iz-1)*deltaz, pot3D(ix+(xx-1)*Ndeltax, iy,iz)
!!$end do
!!$write(135,*)
!!$end do
!!$close(134)
...
write(*,*)'TRANSFORMING THE POTENTIAL with',Nomp,'threads',', ikyz',iyz
...
do xx=1,ncx_d
   ...
   write(*,*)xx,'ref_index=',ref_index,ref_index-nsolv+1,ref_index+nsolc
   write(*,*)xx,'kv_max=',kv_max(iyz,imat(xx)),'kc_min=',kc_min(iyz,imat(xx))

   allocate(A(NM(xx),NM(xx)))
   
   kappax=0.0d0
   !if(chtype .eq. 'n')kappax=kc_min(iyz,imat(xx))
   !if(chtype .eq. 'p')kappax=kv_max(iyz,imat(xx))
...
!  n_bose_f2=1.0_dp/(EXP((Nop_f2*Eop)/(BOLTZ*TEMP))-1.0_dp)
!  n_bose_f3=1.0_dp/(EXP((Nop_f3*Eop)/(BOLTZ*TEMP))-1.0_dp)
...
call omp_set_num_threads(min(Nomp,Nop)) !!! this sets the environement variable OMP_NUM_THREADS 
!write(*,*)'Nsub',Nsub



  ALLOCATE(sigma_greater_ph(1:Nop,1:NMAX,1:Ncx_d))
  ALLOCATE(sigma_lesser_ph(1:Nop,1:NMAX,1:Ncx_d))
  ALLOCATE(sigma_r_ph(1:Nop,1:NMAX,1:Ncx_d))
  ALLOCATE(sigma_greater_ph_prev(1:Nop,1:NMAX,1:Ncx_d))
  ALLOCATE(sigma_lesser_ph_prev(1:Nop,1:NMAX,1:Ncx_d))
...
if(phonon)then!(dac > 0.0_dp .or. dop_g > 0.0_dp)then
     ...
     derror_old=0.0_dp
     omega=0.0_dp
     ALLOCATE(g_lesser(Nop,NMAX,Ncx_d),g_greater(Nop,NMAX,Ncx_d))
     DO WHILE((SCBA_scalar.gt.SCBA_tolerance).and.(SCBA_iter.lt.SCBA_max_iter))
        ...
        do iyz=1,NKYZ !loop over kyz    
           ...
           !$omp degeneracy,w,temp,trans,g_lesser,g_greater,form_factor,flag)
       ...
       !$omp do
        ...
        call RGF(NMAX,flag(nee,iyz),EN,mus,mud,Hi(1:NMAX,1:NMAX,1:Ncx_d,iyz),&
             sigma_lesser_ph_prev(nee,1:NMAX,1:Ncx_d),sigma_greater_ph_prev(nee,1:NMAX,1:Ncx_d),sigma_r_ph_prev(nee,1:NMAX,1:Ncx_d),&
           ...
           do xx=1,ncx_d
              do pp=1,nm(xx)
                 do nn=1,nm(xx)
                 !   !$omp critical
                    g_lesser (nee,pp,xx)=g_lesser (nee,pp,xx)+&
                         degeneracy(iyz)/dble(NCY*NCZ)*g_lesser_diag_local(nn,nn,xx) * form_factor(iyz,imat(xx))%F(nn,pp)
                 !   !$omp end critical
                 !   !$omp critical
                    g_greater(nee,pp,xx)=g_greater(nee,pp,xx)+&
                         degeneracy(iyz)/dble(NCY*NCZ)*g_greater_diag_local(nn,nn,xx)* form_factor(iyz,imat(xx))%F(nn,pp)
                 !   !$omp end critical
                 end do
              end do
           end do
  ...
  !$omp parallel default(none)  private(nee,ii,pp) shared(Nop,Nop_g,sigma_lesser_ph,sigma_greater_ph,sigma_r_ph, &
  !$omp g_lesser,g_greater,Dop_g,n_bose_g,Dac,ncx_d,nm)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     IF(nee.le.Nop_g)THEN
      ...
      !  !$omp atomic      
                 sigma_lesser_ph(nee,pp,ii)=sigma_lesser_ph(nee,pp,ii)+&
                      Dop_g*(n_bose_g+1.0d0)*g_lesser(nee+Nop_g,pp,ii)
      !  !$omp atomic
                 sigma_greater_ph(nee,pp,ii)=sigma_greater_ph(nee,pp,ii)+&
                      Dop_g*(n_bose_g)*(g_greater(nee+Nop_g,pp,ii))
              END DO
           END DO
     ...
     ELSE
      ...
      !  !$omp atomic
                 sigma_lesser_ph(nee,pp,ii)= sigma_lesser_ph(nee,pp,ii)+&
                      Dop_g*(n_bose_g+1.0d0)*g_lesser(nee+Nop_g,pp,ii)+&   
                      Dop_g*(n_bose_g)*g_lesser(nee-Nop_g,pp,ii) 
      !  !$omp atomic
                 sigma_greater_ph(nee,pp,ii)=sigma_greater_ph(nee,pp,ii)+&
                      Dop_g*(n_bose_g)*(g_greater(nee+Nop_g,pp,ii))+&
                      Dop_g*(n_bose_g+1.0d0)*(g_greater(nee-Nop_g,pp,ii))
              END DO
           END DO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     DO ii=1,Ncx_d
        DO pp=1,NM(ii)       
      !  !$omp atomic
           sigma_lesser_ph (nee,pp,ii)=sigma_lesser_ph (nee,pp,ii)+Dac*g_lesser (nee,pp,ii) 
      !  !$omp atomic
           sigma_greater_ph(nee,pp,ii)=sigma_greater_ph(nee,pp,ii)+Dac*g_greater(nee,pp,ii) 
        END DO
     END DO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     DO ii=1,Ncx_d
        DO pp=1,NM(ii)
        !!$omp atomic
           sigma_lesser_ph(nee,pp,ii)=DCMPLX(0.0_dp*dble(sigma_lesser_ph(nee,pp,ii)),abs(aimag(sigma_lesser_ph(nee,pp,ii))))
        !!$omp atomic
           sigma_greater_ph(nee,pp,ii)=DCMPLX(0.0_dp*dble(sigma_greater_ph(nee,pp,ii)),-abs(aimag(sigma_greater_ph(nee,pp,ii))))
        !!$omp atomic
           sigma_r_ph(nee,pp,ii)=DCMPLX(0.0_dp,-0.5_dp*(abs(aimag(sigma_greater_ph(nee,pp,ii)))&
                +abs(aimag(sigma_lesser_ph(nee,pp,ii))))) ! SOLO PARTE IMMAGINARIA
        end do
     end do
  ...
  do nee=1,Nop
     DO ii=1,Ncx_D
        SCBA_f(1:NM(ii))=SCBA_f(1:NM(ii))+((dimag(sigma_lesser_ph(nee,1:NM(ii),ii)-sigma_lesser_ph_prev(nee,1:NM(ii),ii)))**2 +&
          (dimag(sigma_greater_ph(nee,1:NM(ii),ii)-sigma_greater_ph_prev(nee,1:NM(ii),ii)))**2   )
          
        SCBA_x(1:NM(ii))=SCBA_x(1:NM(ii))+((dimag(sigma_lesser_ph(nee,1:NM(ii),ii)))**2 + (dimag(sigma_greater_ph(nee,1:NM(ii),ii)))**2)
        
        derror(nee,ii)=     abs(sum(dimag(sigma_lesser_ph(nee,1:NM(ii),ii)-sigma_lesser_ph_prev(nee,1:NM(ii),ii))))/dble(NM(ii)) 
        
        !if (derror(nee,ii)-derror_old(nee,ii) > 0.0_dp) omega(nee,ii)=0.0_dp!-0.1_dp
...
!close(657+ee*1000)
  ...
  DO nee=1, Nop
     DO ii=1,Ncx_D
        sigma_lesser_ph_prev(nee,1:NM(ii),ii)=sigma_lesser_ph_prev(nee,1:NM(ii),ii) + &
             (1.0d0-omega(nee,ii))*(sigma_lesser_ph(nee,1:NM(ii),ii)-sigma_lesser_ph_prev(nee,1:NM(ii),ii))     
        sigma_greater_ph_prev(nee,1:NM(ii),ii)=sigma_greater_ph_prev(nee,1:NM(ii),ii) + &
             (1.0d0-omega(nee,ii))*(sigma_greater_ph(nee,1:NM(ii),ii)-sigma_greater_ph_prev(nee,1:NM(ii),ii))
        sigma_r_ph_prev(nee,1:NM(ii),ii)=sigma_r_ph_prev(nee,1:NM(ii),ii) + &
             (1.0d0-omega(nee,ii))*(sigma_r_ph(nee,1:NM(ii),ii)-sigma_r_ph_prev(nee,1:NM(ii),ii))                   
     END DO
  END DO
...
!stop
  ...
  do nee=1,Nop
     ...
     if(phonon)then
        call RGF(NMAX,flag(nee,iyz),EN,mus,mud,Hi(1:NMAX,1:NMAX,1:Ncx_d,iyz),&
             0.0_dp*sigma_lesser_ph_prev(nee,1:NMAX,1:Ncx_d),0.0_dp*sigma_greater_ph_prev(nee,1:NMAX,1:Ncx_d),0.0_dp*sigma_r_ph_prev(nee,1:NMAX,1:Ncx_d),&
             g_lesser_diag_local(1:NMAX,1:NMAX,1:Ncx_d),g_greater_diag_local(1:NMAX,1:NMAX,1:Ncx_d),g_r_diag_local(1:NMAX,1:NMAX,1:Ncx_d),&
             tr,tre,cur(ee,nee,1:Ncx_d-1,iyz))
      !  !$omp atomic
...
do nee=1,Nop
   do ee=1,Nsub
     emin_local=en_global(ee) !minimo locale
        EN=emin+emin_local+dble(nee-1)*Eop
        do xx=1,ncx_d
           write(10,*)(xx-1)*ac1*1.0d7,EN,pdos(ee,nee,xx,iyz)!-ndos(ee,nee,xx)+2.0_dp*ldos(ee,nee,xx)
  ...
  do xx=1,ncx_d-1
     ...
     do ee=1,Nsub
        do nee=1,Nop
             emin_local=en_global(ee) !minimo locale
             EN=emin+emin_local+dble(nee-1)*Eop
            ! if(EN.ge.subband(nsolv+1,xx,iyz)-epsilon.or.EN.le.subband(nsolv,xx,iyz)+epsilon)then
...
!!! NOTA: Sigma deve essere cambiata in matrice (nm,nm,nx)
  implicit none
  integer i,j,l,nmax,ff
  real(dp) :: E,mul,mur,tr,tre
  real(dp) :: cur(Ncx_d-1)
  COMPLEX(dp) :: sigma_lesser_ph(nmax,Ncx_d),sigma_greater_ph(nmax,Ncx_d),sigma_r_ph(nmax,Ncx_d),Gn(nmax,nmax),Gp(nmax,nmax)
  COMPLEX(dp) :: Hii(nmax,nmax,ncx_d),ldos(nmax,nmax,ncx_d),ndens(nmax,nmax,ncx_d),pdens(nmax,nmax,ncx_d)
  COMPLEX(dp) :: H00(nmax,nmax),H10(nmax,nmax),A(nmax,nmax),B(nmax,nmax),C(nmax,nmax),Id(nmax,nmax),G00(nmax,nmax),GN0(nmax,nmax)
  COMPLEX(dp) :: sig(nmax,nmax),sigmal(nmax,nmax),sigmar(nmax,nmax),Gl(nmax,nmax,Ncx_d),Gln(nmax,nmax,Ncx_d),Glp(nmax,nmax,Ncx_d)
  COMPLEX(dp) :: z
...
!     write(*,*)E,'traccia',traccia(dimag(- G00(:,:))),traccia(dimag(- G00(1:nm(l),1:nm(l))))
!     tr=0.0_dp
!     do i=1,nm(l)
!        tr=tr+dimag(- G00(i,i))
!     end do
...
!  sig(1:nm(l),1:nm(l))=0.5_dp*(sig(1:nm(l),1:nm(l))-transpose(dconjg(sig(1:nm(l),1:nm(l)))))
...
!!!  Gl(:,:,l)=0.5_dp*(Gp-Gn+Gl(:,:,l)+transpose(dconjg(Gl(:,:,l))))
...
!     C(1:nm(l),1:nm(l))=0.5_dp*(C(1:nm(l),1:nm(l))-transpose(dconjg(C(1:nm(l),1:nm(l)))))
     ...
     !Gn(1:nm(l),1:nm(l))=0.5_dp*(Gn(1:nm(l),1:nm(l))-transpose(dconjg(Gn(1:nm(l),1:nm(l)))))
...
!     C(1:nm(l),1:nm(l))=0.5_dp*(C(1:nm(l),1:nm(l))-transpose(dconjg(C(1:nm(l),1:nm(l)))))
     ...
     !Gp(1:nm(l),1:nm(l))=0.5_dp*(Gp(1:nm(l),1:nm(l))-transpose(dconjg(Gp(1:nm(l),1:nm(l)))))
...
!     write(*,*)E,'somma ',tre     
!     end if
!     if( traccia(dimag(-ldos(:,:,l))) < -1.0d-3 &
!          .or.  traccia(dimag(-pdens(:,:,l) )) <-1.0d-3 &
!          .or.  traccia(dimag( ndens(:,:,l) )) <-1.0d-3 )then
!        ff=-l
     end if
...
elseif ( ff /= 0 ) then
  ...
  subroutine invert_OLD(A,n,np)
      ...
      complex(dp), dimension(np*np) :: work
...
!  call invert(GBB,nm,nm)
  ...
  !if (traccia(dimag(G00))>0.0_dp)G00=0.0_dp
