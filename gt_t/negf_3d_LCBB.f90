...
call omp_set_num_threads(Nomp)!(NCX_D) !!! this set the environement varia
...
!              do pp=1,nm(xx)
                 do nn=1,nm(xx)
                    ...
                    g_lesser (nee,1:nm(xx),xx)=g_lesser (nee,1:nm(xx),xx)+&
                         degeneracy(iyz)/dble(NCY*NCZ)*g_lesser_diag_local(nn,nn,xx) * form_factor(iyz,imat(xx))%F(nn,1:nm(xx))

                    g_greater(nee,1:nm(xx),xx)=g_greater(nee,1:nm(xx),xx)+&
                         degeneracy(iyz)/dble(NCY*NCZ)*g_greater_diag_local(nn,nn,xx)* form_factor(iyz,imat(xx))%F(nn,1:nm(xx))
